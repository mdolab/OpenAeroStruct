...
surface = {
    ...
    "sec_name": ["sec0","sec1"],  # names of the individual sections. Each section must be named and the list length must match the specified number of sections.
    ...
    "chord_cp": [np.array([1,1]),np.array([1,1])], #The chord B-spline parameterization for EACH SECTION. The list length must match the specified number of sections.
    "twist_cp": [np.zeros(2),np.zeros(2)], #The twist B-spline parameterization for EACH SECTION. The list length must match the specified number of sections.
    ...
    "ny" : [21,21], #Number of spanwise points for each section. The list length must match the specified number of sections. (required if using the built-in mesh generator)
...
multi_geom_group = MultiSecGeometry(surface=surface,joining_comp=True,dim_constr=[np.array([1,0,0]),np.array([1,0,0])])
