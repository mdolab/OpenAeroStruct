...
def generateMesh(sections,data,bPanels,cPanels,symmetry=True,controlPoints=False,generatePlots=False,plotOptions={}):
	"""
	...
	if controlPoints:
		...
		controlPointsX, qControlPointsX, controlPointsY, chordDistGeom, chordDistCont = calcControlPoints(panelGeomY,panelGeomX,tquartX,panelQC,bPanels,cPanels)
	if generatePlots:
...
if __name__ == "__main__":
	...
	cPanels = 16
	data = np.array([[0.39319,14.4533,.466822,75],[0,5.6829,1.86524,65]])
	mesh = generateMesh(sections,data,bPanels,cPanels,True,False,True,plotOptions)
