...
class Test(unittest.TestCase):
    def test(self):
        ...
        indep_var_comp.add_output(
            "re",
            val=np.array(
                [0.348 * 295.07 * 0.85 * 1.0 / (1.43 * 1e-5), 1.225 * 340.294 * 0.64 * 1.0 / (1.81206 * 1e-5)]
            ),
            units="1/m",
        )
        indep_var_comp.add_output("rho", val=np.array([0.348, 1.225]), units="kg/m**3")
        ...
        assert_rel_error(
            self,
            prob["wing.geometry.t_over_c_cp"],
            np.array([0.10247881, 0.08207636, 0.11114547, 0.13114547, 0.10207636, 0.09365598]),
            1e-5,
        )
